...
const char * const PIVX_CONF_FILENAME = "adapolice.conf";
const char * const PIVX_PID_FILENAME = "adapolice.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// AdaPolice only features
// Masternode
...
#else
    const char* pszModule = "adapolice";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\adapolice
// Windows >= Vista: C:\Users\Username\AppData\Roaming\adapolice
// Mac: ~/Library/Application Support/adapolice
// Unix: ~/.adapolice
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AdaPolice";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\adapoliceParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\adapoliceParams
    // Mac: ~/Library/Application Support/adapoliceParams
    // Unix: ~/.adapolice-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AdaPoliceParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "AdaPoliceParams";
#else
    // Unix
    return pathRet / ".adapolice-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty adapolice.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override adapolice.conf
