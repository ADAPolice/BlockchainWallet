...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter APC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no APC: URI
...
{
    // Convert adapolice:// to adapolice:
    //
    //    Cannot handle this later, because adapolice:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "adapolice.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for adapolice.lnk
...
{
    return GetAutostartDir() / "adapolice.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a adapolice.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AdaPolice\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the adapolice app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add adapolice app to startup item list
