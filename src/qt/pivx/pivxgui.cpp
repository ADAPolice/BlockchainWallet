...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("AdaPolice client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("AdaPolice"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("AdaPolice"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
