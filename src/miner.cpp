...
{
    // double check that there are no double spent zAPC spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zAPC serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zAPC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zAPC check to not include duplicated serials in the same block.
