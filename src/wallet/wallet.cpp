...
/**
 * Fees smaller than this (in uAPC) are considered zero fee (for transaction creation)
        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zAPC seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zAPC upgrade is active.
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-APC-address
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in APC) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of AdaPolice"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart AdaPolice to complete"));
            return nullptr;
        } else {
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zAPCWallet
        uiInterface.InitMessage(_("Syncing zAPC wallet..."));
