...
bool CWallet::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save zAPCseed to wallet";
    }
    //the use case for this is no password set seed, mint dzAPC,
...
{
    //If this is a zapwallettx, need to read zAPC
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zAPC output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to APC
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zAPC was successful
        ...
        CTxDestination * changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zAPC"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zAPC public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zAPC used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zAPC mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to APC address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zAPC in the future, for now it's useless,
