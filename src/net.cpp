...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (AdaPolice users should be well used to small numbers with many decimal places by now :)
...
void CheckOffsetDisconnectedPeers(const CNetAddr& ip)
{
    ...
    if (setOffsetDisconnectedPeers.size() >= MAX_TIMEOFFSET_DISCONNECTIONS) {
        ...
        std::string strWarn2 = _("Please check that your computer's date and time are correct! If your clock is wrong AdaPolice will not work properly.");
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "AdaPolice " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AdaPolice is probably already running."), addrBind.ToString());
        else
