...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or adapolice.conf setting gen)\n"
...
UniValue prioritisetransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_uAPC / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in uAPC) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in uAPC); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in uAPC)\n"
    ...
    //     throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "APC is not connected!");

    // if (IsInitialBlockDownload())
    //     throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "APC is downloading blocks...");
