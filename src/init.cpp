...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all APC specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that AdaPolice is running in a usable environment with all
...
#endif
    LogPrintf("AdaPolice version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize adapolice.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. AdaPolice is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single AdaPolice process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. AdaPolice is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //AdaPolice specific: zerocoin and spork DB's
                ...
                // AdaPolice: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zAPC if already reindexed
