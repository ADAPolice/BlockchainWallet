...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between APC and zAPC
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // APC: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
