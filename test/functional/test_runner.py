...
def main():
    ...
    tmpdir = "%s/adapolice_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and adapoliced must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, keep_cache=False):
    # Warn if adapoliced is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "adapoliced"]) is not None:
            print("%sWARNING!%s There is already a adapoliced process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/adapoliced' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/adapolice-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie adapoliceds, we can apply a
...
class RPCCoverage():
    ...
    commands per `adapolice-cli help` (`rpc_interface.txt`).
