...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected APC and zAPC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zAPCsupply']:
                assert_equal(node['zAPCsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check APC and zAPC supply at the beginning
        # ------------------------------------------
        # zAPC supply: 2 coins for each denomination
        ...
        # APC supply: block rewards minus burned fees for minting
        ...
        # Verify that APC and zAPC supplies were properly updated after the spends and reorgs
        self.log.info("Check APC and zAPC supply...")
