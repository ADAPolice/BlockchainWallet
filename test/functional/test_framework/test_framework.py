...
class PivxTestFramework():
    """Base class for a adapolice test script.

    Individual adapolice test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave adapoliceds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop adapoliceds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing adapoliced/adapolice-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use adapolice-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: adapoliceds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a adapoliced test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple adapoliced test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'adapoliced exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "adapoliced should have exited with an error"
                else:
                    assert_msg = "adapoliced should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as adapoliced's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in adapolice.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in adapolice.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "adapoliced"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run adapoliceds:
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 APC) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 APC), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 APC), change = 6750 - 6666 - fee
    ...
    ### adapolice Specific TestFramework ###
    ###################################
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 APC worth of zerocoins
    ...
    def get_prevouts(self, node_id, utxo_list, zpos=False, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zAPC-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zAPC prevScript is replaced with serialHash hex string.
        """
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zAPC inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(PivxTestFramework):
    ...
    Sets up some adapoliced binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "adapoliced"),
                          help="adapoliced binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "adapoliced"),
                          help="adapoliced binary to use for reference nodes (if any)")
